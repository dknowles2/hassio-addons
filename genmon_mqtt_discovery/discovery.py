...
parser.add_option(
    "--generator_device_name", dest="generator_device_name", default="Generator"
)
parser.add_option(
    "--generator_manufacturer", dest="generator_manufacturer", default="Generac"
)
parser.add_option(
    "--generator_model", dest="generator_model", default="Generic Generator"
)
...
class Discovery:
    ...
    def _generate_sensors(self):
        return [
            ...
            self.generator_sensor(
                engine("Output Power (Single Phase)"), "Engine Output Power", **power()
            ),
            self.generator_binary_sensor(
                outage("System in Outage"),
                "Power Outage",
                payload_off="No",
                payload_on="Yes",
            ),
            ...
            self.generator_sensor(
                maintenance("Service/Total Run Hours"),
                "Total Run Time",
    ...
    def _sensor(self, component, state_topic, name, device, **kwargs):
        if self.opts.genmon_mqtt_root_topic:
            state_topic = (
                self.opts.genmon_mqtt_root_topic.rstrip("/") + "/" + state_topic
            )
    ...
    def generator_binary_sensor(self, topic, name, **kwargs):
        return self._sensor("binary_sensor", topic, name, self._generator(), **kwargs)
